...
async function callStructured<S extends z.ZodTypeAny>(
  ...
  // A single turn, not the original prompt + the full first response as two
  // separate turns — that doubled the token count of what's already a large
  // call (evidence-heavy prompts), landing it in the same per-minute
